 ...
 * @param bColor 背景色
 */
void LCD_ShowChar(uint16_t x, uint16_t y, char chr, uint16_t size, uint16_t fColor, uint16_t bColor)
{
    uint8_t *pfont = NULL;
    uint8_t temp = 0;
    uint16_t color = 0;
    ...
    for (uint8_t h = 0; h < cHigh; h++)                                         // 遍历字符的高度
    {
        for (uint8_t w = 0; w < size / 2; w++)                                  // 遍历字符的宽度
        {
            ...
            {
                color = (temp & 0x01) ? fColor : bColor;                        // 获取像素点的颜色
                LCD_DrawPoint(x + w, y + k + 8 * h , color);
                temp >>= 1;
            }
        }
    }
 ...
 * @param bColor 背景色
 */
void LCD_ShowString(uint16_t x, uint16_t y, char *str, uint16_t size, uint16_t fColor, uint16_t bColor)
{
    uint16_t x0 = x;
    for (uint16_t i = 0; str[i] != '\0'; i++)
    {
        ...
        LCD_ShowChar(x, y, str[i], size, fColor, bColor);
        x += (size / 2);
    }
}
 ...
 * @param bColor 背景色
 */
void LCD_ShowChinese(uint16_t x, uint16_t y, char *chinese, uint16_t size, uint16_t fColor, uint16_t bColor)
{
                ...
                for (uint16_t h = 0; h < cHigh; h++)                             // 遍历字符的高度
                {
                    for (uint16_t w = 0; w < size; w++)                         // 遍历字符的宽度
                    {
                        ...
                        for (uint16_t k = 0; k < 8; k++)                        // 一个字节8个像素点
                        {
                            color = (temp & 0x01) ? fColor : bColor;            // 获取像素点的颜色
                            // ((i + 1) / 3)定位到第几个汉字
                            LCD_DrawPoint(x + w + ((i + 1) / 3 - 1) * size, y + k + 8 * h , color);
                            temp >>= 1;
                        }
                    }
                }
 ...
 */
void LCD_ShowNumber(uint16_t x, uint16_t y, int num, uint16_t size, uint16_t fColor, uint16_t bColor, DataFormat dataFormat)
{
    char str[20] = {0};
    ...
    LCD_ShowString(x, y, str, size, fColor, bColor);
}
 ...
 * @param dataFormat 格式化结构体
 */
void LCD_ShowDecimal(uint16_t x, uint16_t y, double decimal, uint16_t size, uint16_t fColor, uint16_t bColor, DataFormat dataFormat)
{
    int num = (int)decimal;
    int temp = num;
    int len = 0;
    ...
    LCD_ShowChar(x1, y, '.', size, fColor, bColor);
    x1 += (size / 2);
 ...
 */
void LCD_Printf(uint16_t x, uint16_t y, uint16_t size, uint16_t fColor, uint16_t bColor, char *format, ...)
{
    char str[256];                                                              // 定义字符数组
    ...
    va_end(arg);                                                                // 结束变量arg
    LCD_ShowString(x, y, str, size, fColor, bColor);                            // OLED显示字符数组（字符串）
}
